...
void for_each_test() {
  std::cout << std::endl
            << "***For each and lambda function test***" << std::endl;
  int tab[] = {1, 2, 3, 4};
  std::cout << "tab looks like : " << tab << std::endl;
  std::for_each(tab, tab + 4, [](int &n) { n++; });
...
void capture_context_test() {
  ...
  std::cout << "Initially : i=" << i << ", j=" << j << ", sum=" << sum()
            << std::endl;
  i++;
  std::cout << "Increasing i: i=" << i << ", j=" << j << ", sum=" << sum()
            << std::endl;
  j++;
  std::cout << "Now increasing j: i=" << i << ", j=" << j << ", sum=" << sum()
            << std::endl;
}
...
void aggregate_test() {
  ...
  Complex c{1.3, 2.8};
  std::cout << "Our complex number is c=(" << c.real << ", " << c.imag << ")"
            << std::endl; // can I write a tuple like that ?
  double r = c.real;
  c.imag += 2.;
  Complex c_nul{0., 0.};
  c = c_nul;
...
void pointeur_test() {
  ...
  int i{5};
  std::cout << "We defined i=" << i << " and &i=" << &i << " is its address"
            << std::endl;
...
// its possible to define the methods outside of the class declaration as well
// like this :
/*
double Complex_m::module() const { // modification des attributs interdite avec
flag const return std::sqrt( real*real + imag*imag );
