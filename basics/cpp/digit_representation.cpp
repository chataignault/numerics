...
Number build_number(unsigned long l) {
  ...
  if (l >= number_base)
    n->next_ = build_number(l / number_base);
  return n;
}
...
/*
Number read_number() {
    std::cin >> std::ws;              // saut des blancs (whitespaces),
<iomanip> nécessaire Number p{nullptr}; while( std::cin.good() ) { int c{
std::cin.get() }; if( std::isdigit( c )) {      // <cctype> nécessaire DigitType
d{ static_cast< DigitType >( c - '0' )};
...
void multiply_number(Number n, unsigned int i, unsigned int carry = 0) {
  ...
  n->digit_ = mul % number_base;
  if (n->next_ != nullptr)
    multiply_number(n->next_, i, mul / number_base);
  else if (mul >= number_base)
    n->next_ = build_number(mul / number_base);
}
...
void test_51() {
  ...
  multiply_number(n, std::numeric_limits<unsigned int>::max(),
