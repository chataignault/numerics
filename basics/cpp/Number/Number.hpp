...
public:
  ...
  Number(unsigned long l) {
    this->first_ =
        new Digit(l); // initialise first_ that is the only attribute of Number
  }
  // Destructeur
  ...
  void swap(Number &n) { std::swap(first_, n.first_); }
  ...
  */
  ...
  void print(std::ostream &out) const { this->first_->print(out); }
...
private:
  ...
  // Un seul chiffre décimal par maillon : l'objectif ici n'est pas la
  // performance
  ...
  struct Digit {
    ...
    Digit(unsigned long l) { // recursive constructor
      ...
      if (l >= number_base) {
        this->next_ =
            new Digit{l / number_base}; // dynamic allocation, new returns the
    ...
    void add(unsigned int n) {
      ...
      if (value >= number_base) {
        if (this->next_ == nullptr) {
          ...
          this->next_->add(value / number_base);
      }
    ...
    void multiply(unsigned int n, unsigned int carry = 0) {
      ...
      if (this->next_ == nullptr) {
        ...
        this->next_->multiply(n, value / number_base);
    }
  };
