...
void test11(unsigned long nb_points) {
  ...
  std::cout << "The approximate value is : " << pi_approx << std::endl
            << "It's absolute difference with the real value of pi is : "
            << std::abs(M_PI - pi_approx) << std::endl;
}
...
void test12(unsigned long nb_points) {
  ...
  std::cout << "The computation time was : " << elapsed_seconds.count()
            << std::endl
            << "Corresponding to an absolute error of : "
            << std::abs(M_PI - pi_approx) << std::endl;
}
...
void test14(unsigned long nb_points) {
  ...
  auto end = std::chrono::steady_clock::now();
  std::chrono::duration<double> elapsed_seconds = end - start;
  std::cout << "The max absolute difference for 10 iterations is : " << max_diff
            << std::endl
            << "The computation time was : " << elapsed_seconds.count()
            << std::endl;
}
...
FloatKind compute_pi(unsigned long nb_points, unsigned int nb_threads) {
  ...
  for (int i{0}; i < nb_threads; i++) {
    std::packaged_task<unsigned long(unsigned long)> task{
        nb_points_inside_unit_circle};
  ...
  FloatKind sum =
      std::accumulate(pi_approximations.begin(), pi_approximations.end(), 0.);
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  /*1.5*/
  /*
  Using double instead of floats leads to an slight increase in computation
  time, the precision is unchanged Using long double has the same effect.
  */
