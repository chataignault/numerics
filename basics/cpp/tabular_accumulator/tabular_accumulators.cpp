#include "tabular_accumulators.h"
...
int main(int argc, char *argv[]) {
  ...
  // flux d'entree peut etre considere comme un conteneur dont on va pouvoir
  // faire la copie
  std::copy(std::istream_iterator<std::string>(data), // iterateur de debut
  ...
  using MonthAccumulator =
      acc::accumulator_set<double, acc::stats<acc::tag::mean, acc::tag::min,
                                              acc::tag::max>>; // defini un type
  // stats sur l'ensemble des données
  std::map<int, MonthAccumulator> statistics;
  auto key_year_month = [](const Measure &m) {
  ...
  for (const auto &[key, value] : statistics) {
    std::cout << month_from_key(key) << "-" << year_from_key(key)
              << " min: " << acc::min(value) << " max: " << acc::max(value)
              << " moy: " << acc::mean(value) << "\n";
  }
}
