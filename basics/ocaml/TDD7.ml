...
let rec cart x l res =
  match l with [] -> res | t :: q -> cart x q ((x, t) :: res)

let rec cartesien liste1 liste2 =
  match liste1 with
  | [] -> []
  | x1 :: q1 ->
      let res = cartesien q1 liste2 in
      cart x1 liste2 res

(*differnce avec : ?*)

let rec ajoute a l acc =
  match l with [] -> acc | b :: q -> (a, b) :: ajoute a q acc

let rec produit l1 l2 =
  match l1 with [] -> [] | a :: q -> ajoute a l2 (produit q l2)

let reconnait aut mot =
  let rec aux mot s =
    match mot with
    | [] -> commun [ s ] aut.terminaux
    | t :: q -> aut.delta.(s).(t) <> -1 && aux q aut.delta.(s).(t)
  in
...
let linearise expr =
  ...
  let rec aux expression =
    ...
    | Etoile expr -> Etoile (aux expr)
  in

  let ne = aux expr in
  (ne, List.rev !lis, !k)

let rec calcule_lang_local expr_l =
  match expr_l with
  | Eps -> { i = []; s = []; f = []; vide = true }
  | Lettre x -> { i = [ x ]; s = [ x ]; f = []; vide = false }
  | Sum (e1, e2) ->
      let l1 = calcule_lang_local e1 and l2 = calcule_lang_local e2 in
      {
        i = fusionne l1.i l2.i;
        s = fusionne l1.s l2.s;
        f = fusionne l1.f l2.f;
...
let calcule_auto_lang_local lang c =
  ...
  let auto =
    { initial = 0; terminaux = !term; delta = Array.make_matrix (c + 1) c (-1) }
  in
  let l = ref lang.i in
  while !l <> [] do
    auto.delta.(0).(List.hd !l - 1) <- List.hd !l;
    (*transitions qui parteent etat init*)
    l := List.tl !l
  done;
...
let automate expr =
  ...
  let resultat =
    {
      initiaux = [ 0 ];
      finaux = auto.terminaux;
      ddelta = Array.make (c + 1) [];
    }
  in
  for i = 0 to c do
    for j = 0 to c - 1 do
      if auto.delta.(i).(j) <> -1 then
        resultat.ddelta.(i) <-
          insere (table.(j), auto.delta.(i).(j)) resultat.ddelta.(i)
    done
  done;
  resultat

let testlin =
  let ne, r, c = linearise test in
  ne
;;
...
let rec equivalent expr =
  match expr with
  | Eps -> Eps
  | Somme (e1, e2) -> begin
      let ep1 = equivalent e1 and ep2 = equivalent e2 in
      match (ep1, ep2) with
      | Vide, Vide -> Vide
      | Vide, _ -> ep2
      | _, Vide -> ep1
