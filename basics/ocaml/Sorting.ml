let rec facto n =
  if n < 0 then raise (Failure "facto")
  else match n with 0 -> 1 | k -> k * facto (k - 1)
...
let rec combien_aux a l c =
  match l with
  | [] -> c
  | h :: t -> (
      match cherche a h with
...
(*Tri par insertion*)
let rec inser_list a l =
  match l with
  | [] -> [ a ]
  | h :: t -> if a < h then a :: h :: t else h :: inser_list a t

let rec tri_inser_aux l1 l2 =
  match l1 with [] -> l2 | h :: t -> tri_inser_aux t (inser_list h l2)
      ...
      for i = 0 to k do
        if t.(i) > t.(k) then begin
          for j = k - 1 downto i do
            t.(j + 1) <- t.(j)
            (*on fait de la place pour inserer la kieme valeur*)
          done;
          t.(i) <- x;
          (*on insere*)
          raise Not_found
...
let recherche_array_trie a t =
  ...
  try
    for k = 0 to Array.length t - 1 do
      let z = (!x + !y) / 2 in
      begin if t.(z) <= a then x := z else y := z
      end;
...
let suppression_array_trie a t =
  let n = compte_array a t in
  let t2 = Array.make (Array.length t - n) t.(0) and m = ref 0 in
  for k = 0 to Array.length t2 - 1 do
    if t.(k) = a then begin
      while t.(k + !m) = a && k + !m <= Array.length t2 - 1 do
        m := !m + 1
      done;
      t2.(k) <- t.(k + !m)
    end
...
let rec insertion_list_triee a l =
  match l with
  | [] -> [ a ]
  | h :: t -> if a < h then a :: h :: t else h :: insertion_list_triee a t

let rec recherche_list_triee a l =
  match l with
  | [] -> false
  | h :: t -> if h = a then true else recherche_list_triee a t

let rec suppression_list_triee a l =
  match l with
  | [] -> []
  | h :: t ->
      if h > a then h :: t
      else if h = a then suppression_list_triee a t
      else h :: suppression_list_triee a t
...
let insertion_pile_triee a p =
  p.liste <- insertion_list_triee a p.liste;
  p

let recherche_pile_triee a p = recherche_list_triee a p.liste

let suppression_pile_triee a p =
  p.liste <- suppression_list_triee a p.liste;
  p
...
let insertion_file_triee a f =
  if min f.avant > a then
    {
      avant = f.avant;
      arriere = inversion (insertion_list_triee a (inversion f.arriere));
    }
  else { avant = insertion_list_triee a f.avant; arriere = f.arriere }

let recherche_file_triee a f =
  recherche_list_triee a f.avant || recherche_list_triee a (inversion f.arriere)

let suppression_file_triee a f =
  {
    avant = suppression_list_triee a f.avant;
    arriere = inversion (suppression_list_triee a (inversion f.arriere));
  }
