let rec element x l =
  match l with [] -> false | h :: t -> x = h || element x t
...
let rec insertion_ens x e =
  match e with
  | [] -> [ x ]
  | h :: t ->
      if h < x then h :: insertion_ens x t
      else begin
        if h = x then t else x :: h :: t;
        o
      end

let rec eliminer x e =
  match e with
  | [] -> []
  | h :: t ->
      if h = x then t
      else begin
        if h > x then e else h :: eliminer x t
      end
...
let etiquette a =
  match a with ArbreVide -> failwith "arbrevide" | Noeud (e, _, _) -> e

let droit a =
  match a with ArbreVide -> failwith "arbrevide" | Noeud (_, _, d) -> d

let rec taille a =
  match a with ArbreVide -> 0 | Noeud (_, g, d) -> 1 + taille g + taille d
...
let rec recherche_abr x a =
  match a with
  | ArbreVide -> false
  | Noeud (e, g, d) ->
      if e < x then recherche_abr x d else e = x || recherche_abr x g

let rec traitement a f =
  match a with
  | ArbreVide -> ArbreVide
...
let tete p = match p.liste with [] -> failwith "pile vide" | h :: t -> h
...
let queue l =
  match l with ListeVide -> failwith "liste vide" | Liste (h, t) -> t

type 'a file = { avant : 'a list; arriere : 'a list }

let enfile a f =
  let f = { avant = a :: f.avant arriere = f.arriere } in
  f

let filevide () = { avant = []; arriere = [] }

let rec defile f =
  match f.arriere with
  | h :: t ->
      f = { avant = f.avant; arriere = t };
      h
  | [] ->
      if f.avant = [] then failwith "file vide"
      else f = { avant = []; arriere = inverser f.avant };
      defile f
