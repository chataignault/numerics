...
let tete l = match l with [] -> failwith "tete" | h :: t -> h
let tete2 = function [] -> failwith "tete2" | h :: t -> h
let queue l = match l with [] -> failwith "queue" | h :: t -> t
let queue2 = function [] -> failwith "queue" | h :: t -> t
let rec longueur l = match l with [] -> 0 | h :: t -> 1 + longueur t
let longueur2 = function [] -> 0 | h :: t -> longueur t + 1
...
let degre p = match p with [] -> 0 | (a, n) :: t -> n

let rec recherche l x =
  match l with [] -> false | a :: t -> a = x || recherche t x

(* on utilise l'ordre d'evaluation de l'operateur || *)

let rec coefficient p i =
  match p with [] -> 0. | (a, n) :: t -> if n = i then a else coefficient t i

let rec somme_elem c p =
  match (c, p) with
  | _, [] -> [ c ]
  | (a, n), (b, m) :: t ->
      if n = m then (a +. b, n) :: t else (b, m) :: somme_elem c t

let rec somme p1 p2 =
  match p1 with [] -> p2 | c :: t -> somme t (somme_elem c p2)

let rec produit_elem c p =
  match (c, p) with
  | _, [] -> []
...
let rec degreH = function [] -> 0 | a :: p -> 1 + degreH p
