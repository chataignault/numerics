let rec amplitude e =
  let n = Array.length e and min = ref 0 and max = ref 0 in
  for i = 1 to n - 1 do
    if e.(i) > e.(!max) then max := i else if e.(i) < e.(!min) then min := i
  done;
  e.(!max) - e.(!min)
...
let plusgrand t =
  ...
  for i = 0 to n - 1 do
    if t.(i) <> !un && t.(i) > !deux then deux := t.(i)
  done;
  (!un, !deux)
