let rec bitfaible x =
  match x with 0 -> [] | _ -> (x mod 2) :: bitfaible (x / 2)
...
let rec insere a lis =
  match lis with
  | [] -> [ a ]
  | b :: q when a <= b -> a :: lis
  | b :: q -> b :: insere a q

let rec tri_insert lis =
  match lis with [] -> [] | a :: q -> insere a (tri_insert q)
...
let tri_insert_vect v =
  ...
  for i = 1 to n - 1 do
    ...
    let m = ref (i - 1) in
    while !m <> 0 && !a < v.(!m) do
      (*attention a l'ordre d'evaluation*)
      m := !m - 1
    done;
...
let croissance liste =
  match liste with
  | [] -> true
  | h :: t ->
      let rec cr_aux x l =
        match l with [] -> true | h :: t -> h >= x && cr_aux h t
      in
      cr_aux h t
  ...
  | h :: t -> (
      ...
      try
        while !l <> [] do
          ...
          | [] -> raise Oui
          | h1 :: t1 ->
              if h1 < !el then raise Non else el := h1;
              l := t1
        done;
        true
      with
...
(*renvoie la liste a l'envers *)

let tete liste = match liste with [] -> failwith "listevide" | h :: t -> h

let rec sing_cart x l =
  match l with [] -> [] | h :: t -> (x, h) :: sing_cart x t
