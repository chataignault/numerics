...
(* on remonte le tableau en conservant combinaison optimale contenue dans la suite d*)

type point = { x : float; y : float }

let longueur polygone i j =
  if j = i + 1 then 0.
  else
    sqrt
      (((polygone.(i).x -. polygone.(j).x) ** 2.)
      +. ((polygone.(i).y -. polygone.(j).y) ** 2.))
      ...
      if j >= i + 2 then begin
        ...
        for k = i to j - 1 do
          let a =
            c.(i).(k)
            +. c.(k + 1).(j)
            +. longueur polygone (i - 1) k
            +. longueur polygone k j
          in
          if a < !cout_min then cout_min := a;
          meilleur_indice := k
        done;
        c.(i).(j) <- !cout_min;
        ks.(i).(j) <- !meilleur_indice
      end
    done
  done;
  ks
...
let ordonnance temps prix =
  ...
  let qui = Array.make n (-1) and combien = Array.make n 0 in
  combien.(0) <- prix.(0);
  for i = 1 to n - 1 do
    let a, b = temps.(i) in
    let j = ref (n - 1) in
    ...
    if snd temps.(i) <= a then begin
      ...
      if x > combien.(i - 1) then qui.(i) <- !j;
      combien.(i) <- x
    end
  done;
  let q = ref (n - 1) in
  let liste_clients = ref [] in
  while !q >= 0 && qui.(!q) = -1 do
    q := !q - 1
  done;
