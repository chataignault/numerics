...
let numerote a =
  let rec aux a n =
    match a with
    | V -> V
    | N (x, ag, ad) -> N (n, aux ag (2 * n), aux ad ((2 * n) + 1))
  in
  aux a 1
...
let rec est_abr a =
  ...
  | N (x, ag, ad) -> (
      ...
      | N (y, agg, agd), N (z, adg, add) ->
          x <= z && y <= x && est_abr adg && est_abr add && est_abr agg
          && est_abr agd)
...
let estabr a =
  let rec aux ar =
    ...
    | N (x, V, V) -> (true, x, x)
    | N (x, ag, V) ->
        let bg, ming, maxg = aux ag in
        (bg && maxg <= x, ming, x)
    | N (x, V, ad) ->
        let bd, mind, maxd = aux ad in
        (bd && x <= mind, x, maxd)
    | N (x, ag, ad) ->
        let bd, mind, maxd = aux ad and bg, ming, maxg = aux ag in
...
let esttas a =
  let rec aux ar =
    ...
    | V -> (true, -1)
    | N (x, ag, ad) ->
        let bg, vg = aux ag and bd, vd = aux ad in
        (bg && bd && x >= vg && x >= vd, x)
  in
  fst (aux a)
...
let recherche a c =
  ...
  in
  let b = aux a in
  if b then (b, !lis) else (b, [])

let rec recherche a c =
  match a with
  | V -> []
  | N (x, ag, ad) when x < c ->
      let resd = recherche ad c in
      if resd = [] then [] else x :: resd
  | N (x, ag, ad) when x > c ->
      let resg = recherche ag c in
      if resg = [] then [] else x :: resg
  | N (x, ag, ad) -> [ x ]
...
let rec construit l = match l with [] -> V | x :: q -> insere x (construit q)

let rec pinfixe a acc =
  match a with
  | V -> acc
  | N (x, ag, ad) ->
      let ld = pinfixe ad acc in
      pinfixe ag x :: ld

let infixe a = pinfixe a []

let tri liste =
  let a = construit liste in
  infixe a
...
let rec fusionne a1 a2 =
  match (a1, a2) with
  | a, V -> a1
  | V, a -> a2
  | N (x1, ag1, ad1), _ -> N (x, a1g, fusionne ad1 a2)

(*quand on le reapplique l'hypothese est toujours validee*)
(*complexite en O de la hauteur de a1*)

let rec extraire supprime a c =
  match a with
  | V -> V
