...
let rec preordre a =
  ...
  | [] -> []
  | R (x, t) :: q ->
      let ls = preordre q in
      (x :: preordre t) @ ls

let prefixe arbre =
  let res = ref [] in
  let rec explore arbre =
    match arbre with
    | [] -> ()
    | R (x, ff) :: q -> begin
        explore q;
...
let adresse x a =
  let rec explore a ind res =
    match a with
    | [] -> (false, [])
    | R (y, f) :: q ->
        if x = y then (true, ind :: res)
        else begin
          let ls = ref f and k = ref 0 in
          if fst (explore q (ind + 1) res) = true then
            snd (explore q (ind + 1) res)
          else
            while !ls <> [] do
              let b = List.hd !ls in
              ls := List.tl !ls;
  ...
  let rec present f =
    ...
    | R (a, ff) :: q ->
        if x = a then (true, [ 0 ])
          ...
          let bff, adff = present ff in
          if bff then (true, 0 :: adff)
          else
            let bq, adqq = present q in
            if bq then (true, (List.hd adqq + 1) :: List.tl adqq)
            else (false, [])
  in
  let b, ad = present [ arbre ] in
  (b, List.tl ad)
...
let qui arbre adresse =
  ...
  while !ad <> [] do
    ...
    ad := List.tl !ad;
    while !k <> 0 do
      if !a = [] then failwith "mauvaise adresse" else a := List.tl !a
    done;
    match !a with [] -> failwith "mauvaise adresse" | R (x, f) :: q -> a := f
  done;
  match !a with [] -> failwith "mauvaise adresse" | R (x, f) :: q -> x
...
let rec transforme foret =
  match foret with
  | [] -> Nil
  | R (x, f) :: q -> Noeud (x, transforme f, transforme q)

let rec taf a =
  match a with Nil -> [] | Noeud (x, u, v) -> R (x, taf u) :: taf v
;;

c;;

let rec vider l1 l2 = match l1 with [] -> l2 | t :: q -> vider q (t :: l2)
let inv l = vider l []
