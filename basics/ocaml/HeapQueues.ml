...
let eval t =
  let n = Array.length t and p = pilevide () in
  for k = 0 to n - 1 do
    match t.(k) with
    | Nb x -> empile x p
...
let eval_poly t =
  ...
  for k = 0 to n - 1 do
    ...
    | Op2 x ->
        empile
          (x (depile p) (depile p))
          p (*attention a l'ordre d'evalutaion a tester*)
...
let tete p = match p.liste with [] -> [] | h :: t -> h
let rec vider l1 l2 = match l1 with [] -> l2 | h :: t -> vider t (h :: l2)
let inverse l = vider l []
let queue p = match inverse p.liste with [] -> [] | h :: t -> h
