...
let arrange t debut fin =
  let pivot = t.(debut) and ind = ref debut in
  for k = debut + 1 to fin do
    if t.(k) < pivot then begin
      incr ind;
      echange t !ind k
    end
  done;
...
let rec tri t i j =
  if i >= j then ()
    (*i=j pas suffisant car p ex si m = i on fait appel i -> i - 1 ce que est impossible*)
  else
    let m = arrange t i j in
    tri t i (m - 1);
    tri t (m + 1) j

(*car le pivot est deja trie*)
...
let verification t a i j =
  let m = ref 0 and n = j - i in
  for k = i to j do
    if t.(k) = a then incr m
  done;
  !m > n / 2
...
let rec elem_maj1_aux t i j =
  if i = j then (true, t.(i))
    ...
    | (false, _), (false, _) -> (false, t.(0))
    | (true, e), (false, _) ->
        if verification t e i j then (true, e) else (false, t.(0))
    | (false, _), (true, e) ->
        if verification t e i j then (true, e) else (false, t.(0))
    | (true, e), (true, f) ->
        if verification t e i j then (true, e)
        else begin
