let rec suite_rec u0 f n = if n = 0 then u0 else f (suite_rec u0 f (n - 1))
let rec suite_rec_term u0 f n = if n = 0 then u0 else suite_rec (f u0) f (n - 1)
let rec facto n = if n = 0 then 1 else n * facto (n - 1)
let rec facto_aux n m = if n = 1 then m else facto_aux (n - 1) (n * m)
let facto_term n = facto_aux n 1
let rec longueur = function [] -> 0 | a :: l -> 1 + longueur l

let rec longueur_aux l n =
  match l with [] -> n | h :: t -> longueur_aux t (n + 1)

let longueur_term l = longueur_aux l 0

(*TYPES SOMMES*)

type couleur = Vert | Bleu | Jaune
