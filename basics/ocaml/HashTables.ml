...
let recherche d k =
  match d.assoc.(k) with Nil -> failwith "non trouve" | Valeur e -> e
...
let insere k a t =
  t.cellule.(t.fonction k) <- insere_aux k a t.cellule.(t.fonction k)

let supprime k t =
  t.cellule.(t.fonction k) <- supprime_aux k t.cellule.(t.fonction k)

(*Application a la memoïsation*)
let rec fibo1_aux n =
  if n = 0 then (1, 1) else match fibo1_aux (n - 1) with a, b -> (b, a + b)

let fibo1 n = match fibo1_aux n with a, b -> a

let rec fibo2_aux n (a, b) =
  if n = 0 then (a, b) else fibo2_aux (n - 1) (b, a + b)
...
let rec fibo3 n =
  try recherche n t with
  | Not_found ->
      t.cellule.(h n) <- (n, fibo2 n) :: t.cellule.(h n);
      fibo2 n
  | _ -> recherche n t

let rec combinaison n p =
  if n = p or p = 0 then 1
  else combinaison (n - 1) (p - 1) + combinaison (n - 1) p

let g (n, p) = n + (p mod 17)
let t2 = tablevide m g

let rec combinaison2 n p =
  if n = p || p = 0 then 1
  else
    try recherche (n, p) t2 with
    | Not_found ->
        t2.cellule.(g (n, p)) <-
          ((n, p), combinaison2 (n - 1) (p - 1) + combinaison2 (n - 1) p)
          :: t2.cellule.(g (n, p));
        recherche (n, p) t2
    | _ -> recherche (n, p) t2
