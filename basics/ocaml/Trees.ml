...
let rec antec_syrac n p =
  if p = 0 then Noeud (n, ArbreVide, ArbreVide)
  else begin
    if (n - 4) mod 6 = 0 then
...
let liste_en_largeur arbre =
  let f = filevide () in
  let rec largeur_aux () =
    (*pas besoin de mettre d'argument car f est deja une variable globale dans la fonction*)
    match defile f with
