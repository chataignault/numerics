...
let rec puissance_complexe_aux a b n =
  match n with
  | 0 -> { re = 1.; im = 0. }
  | 1 -> b
...
let rec somme_puiss_aux w p n s =
  match n with
  | 1 -> somme s { re = 1.; im = 0. }
  | m ->
      somme_puiss_aux w p (n - 1) (somme s (puissance_complexe w ((n - 1) * p)))
...
let rec recherche l a =
  match l with [] -> false | h :: t -> h = a || recherche t a

let rec recherche2_aux l a b =
  match l with [] -> b | h :: t -> recherche2_aux t a (b || h = a)
...
let enfile a l =
  if l.longueur = lmax then failwith "file pleine"
  else (
    l.tab.((l.tete + l.longueur) mod lmax) <- a;
    l.longueur <- l.longueur + 1)

let rec enfilage n l =
  if n = 0 then failwith "ok" else enfile (lmax - n) l;
  enfilage (n - 1) l

let defile f =
  if f.longueur = 0 then failwith "file vide" else f.longueur <- f.longueur - 1;
...
let rec il_existe p l =
  match l with [] -> false | h :: t -> p h || il_existe p t

let rec pour_tout p l =
  match l with [] -> true | h :: t -> p h && pour_tout p t

type ('a, 'b) tablehach = {
  cellule : ('a * 'b) list array;
  fonction : 'a -> int;
}
...
let rec insere_aux k a l =
  ...
  | [] -> [ (k, a) ]
  | (q, b) :: t ->
      if q = k then insere_aux k a t else (q, b) :: insere_aux k a t

let insere k a h =
  h.cellule.(h.fonction k) <- insere_aux k a h.cellule.(h.fonction k)

type 'a arbreb = ArbreVide | Noeud of 'a * 'a arbreb * 'a arbreb

let estVide a = a = ArbreVide

let rec parcourspre a =
  match a with
  | ArbreVide -> ()
  | Noeud (a, g, d) ->
      print_int a;
...
let rec antec_syrac n p =
  ...
  | p ->
      if n mod 6 = 4 then
        Noeud (n, antec_syrac (2 * n) (p - 1), antec_syrac ((n - 1) / 3) (p - 1))
...
let rec fusion liste1 liste2 =
  ...
  | _, [] -> liste1
  | h1 :: t1, h2 :: t2 ->
      if h1 < h2 then h1 :: fusion t1 liste2 else h2 :: fusion liste1 t2
...
let rec trifusion liste =
  ...
  | [ a ] -> liste
  | _ ->
      let liste1, liste2 = divise liste in
      fusion (trifusion liste1) (trifusion liste2)

let rec exp x n =
  if n = 0 then 1
  else
    let y = exp (x * x) (n / 2) in
    if n mod 2 = 0 then y else x * y

let rec partition pivot liste =
  match liste with
  | [] -> ([], [])
  | h :: t ->
      let liste1, liste2 = partition pivot t in
      if h < pivot then (h :: liste1, liste2) else (liste1, h :: liste2)

let rec trirapide liste =
  match liste with
  | [] -> []
  | h :: t ->
      let liste1, liste2 = partition h t in
      trirapide liste1 @ (h :: trirapide liste2)

let rec vider liste1 liste2 =
  match (liste1, liste2) with
  | [], _ -> liste2
  | _, [] -> liste1
